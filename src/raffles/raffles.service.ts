...
export class RafflesService {
  ...
  async updateRaffleStatus(
    id: number,
    updateRaffleDto: Prisma.RaffleUpdateInput,
  ): Promise<Raffle> {
    const raffle = await this.findRaffle(id);
    if (!raffle) throw new HttpException('Raffle Not Found', 404);
    if (updateRaffleDto.secretToken !== raffle.secretToken)
      throw new HttpException('Invalid Secret Token', 498);
  ...
  async chooseRaffleWinner(
    ...
    const updatedParticipant =
      await this.participantService.updateParticipantWinner(id);
